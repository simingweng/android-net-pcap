...
public abstract class JMemory {
     ...
     *
     * @param size number of bytes to pre-allocate allocate
     */
     ...
     *
     * @param type type of memory allocation model
     */
     ...
     *
     * @throws IllegalStateException if peered object is not initialized this unchecked exception will
     ...
     * </p>
     *
     * @param address native memory address to use in the disposable
     * @param size    the size
     ...
     *
     * @param peer The ByteBuffer whose allocated native memory we want to peer with.
     *             The ByteByffer must be if direct buffer type which can be checked
     *             using ByteBuffer.isDirect() call.
     * @return the int
     * @throws PeeringException the peering exception {@link #cleanup()} method to ensure that
     ...
     *
     * @param peer the object whose allocated native memory we want to peer with
     * @return the int
     */
     ...
     *
     * @param peer   object memory block to peer with
     * @param offset offset into the memory block
     * @param length amount of memory to peer with
     * @return the int
     * @throws IndexOutOfBoundsException if the specified memory offset and length have negative or out of
     ...
     *
     * @param size size in bytes that is smaller then existing size
     */
     ...
     * memory to textual hexdump.
     *
     * @param length  maximum number of bytes to dump to hex output
     * @param address flag if set to true will print out address offset on every line
     * @param text    flag if set to true will print out a text characters at the end of
     *                everyline
     * @param data    flag if set to true will print out raw HEX data on every line
     * @return multi-line hexdump of the entire memory region
     */
     ...
     *
     * @param buffer    source buffer
     * @param srcOffset starting offset into the byte array
     * @param length    number of bytes to copy
     * @param dstOffset starting offset into memory buffer
     * @return number of bytes copied
     */
     ...
     *
     * @param src       source buffer
     * @param dstOffset offset into our memory location
     * @return actual number of bytes that was copied
     */
     ...
     * properly as well.
     *
     * @param memory memory block to transfer the ownership from
     ...
     *
     * @param buffer destination buffer starting offset in byte array
     * @return number of bytes copied
     */
     ...
     * @param srcOffset starting offset in memory
     * @param length    number of bytes to copy
     * @param dstOffset starting offset in byte array
     * @return number of bytes copied
     */
    protected int transferTo(byte[] buffer,
                             int srcOffset,
