...
public class WinPcap extends Pcap {
     ...
     *
     * @param source will contain the complete source string wen the function returns
     * @param type   its value tells the type of the source we want to created
     * @param host   an user-allocated buffer that keeps the host (e.g. "foo.bar.com")
     *               we want to connect to. It can be NULL in case we want to open an
     *               interface on a local host
     * @param port   an user-allocated buffer that keeps the network port (e.g. "2002")
     *               we want to use for the RPCAP protocol. It can be NULL in case we
     *               want to open an interface on a local host.
     * @param name   an user-allocated buffer that keeps the interface name we want to
     ...
     * @param errbuf buffer that will contain the error message (in case there is one).
     ...
     * </p>
     *
     * @param source  a char* buffer that keeps the 'source localtion', according to the
     ...
     * @param auth    a pointer to a pcap_rmtauth structure. This pointer keeps the
     ...
     * @param alldevs a list of all the network interfaces
     * @param errbuf  error message (in case there is one)
     ...
     *
     * @param program bpf filter
     * @param caplen  length of the captured packet
     * @param len     length of the packet on the wire
     ...
     * @param buf     buffer containing packet data
     * @return snaplen of the packet or 0 if packet should be rejected
     * @see #offlineFilter(PcapBpfProgram, PcapHeader, java.nio.ByteBuffer)
     * @see #offlineFilter(PcapBpfProgram, PcapHeader, org.jnetpcap.nio.JBuffer)
     * @deprecated replaced by
     * {@link #offlineFilter(PcapBpfProgram, PcapHeader, org.jnetpcap.nio.JBuffer)}
     */
    @Deprecated
    public static native int offlineFilter(PcapBpfProgram program,
                                           PcapPktHdr header,
     ...
     * </ul>
     *
     * @param source  The source name has to include the format prefix according to the
     ...
     *                </ul>
     * @param snaplen length of the packet that has to be retained. For each packet
     ...
     *                packet are stored.
     * @param flags   keeps several flags that can be needed for capturing packet
     * @param timeout read timeout in milliseconds. The read timeout is used to arrange
     ...
     * @param auth    a pointer to a 'struct pcap_rmtauth' that keeps the information
     ...
     *
     * @param size size of the sendqueue to allocate
     ...
     * associated with it.
     *
     * @param queue the queue to free up
     */
     ...
     * @throws java.io.IOException any IO errors produced by the appendable
     */
     ...
     * @param maxpackets maximum number of packets to store
     ...
     *
     * @param sync if sync is nonzero, the function blocks until the dump is
     ...
     *
     * @param queue queue containing the data to be sent
     * @param synch if it is non-zero, the packets are sent respecting the timestamps,
     ...
     * buffer by default.
     *
     * @param dim specifies the size of the buffer in bytes
