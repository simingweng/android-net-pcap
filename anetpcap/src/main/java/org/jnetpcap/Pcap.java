 ...
 * use {@link #dispatch(int, org.jnetpcap.packet.PcapPacketHandler, Object)} method, which works
 * very similarly to {@link #loop(int, org.jnetpcap.packet.PcapPacketHandler, Object)}. You can also
 * use {@link #next(PcapHeader, org.jnetpcap.nio.JBuffer)} and
 * {@link #nextEx(PcapHeader, org.jnetpcap.nio.JBuffer)} methods which will deliver 1 packet at a
 * time.
 * </p>
...
public class Pcap {
     ...
     * </p>
     *
     * @param snaplen  generate code to truncate packets to this length upon a match
     * @param dlt      the first header type within the packet, or data link type of the
     *                 interface
     * @param program  initially empty, but after the method call will contain the
     ...
     *
     * @param name data link type name
     ...
     * </ul>
     *
     * @param alldevs the list is filled in with <code>PcapIf</code> interface objects;
     *                the list must not be immutable
     * @param errbuf  error buffer containing error message as a string on failure
     ...
     *
     * @param alldevs is set to point to the first element of the list; each element of
     *                the list is of type PcapIf
     * @param errbuf  error buffer containing error message as a string on failure
     * @see #freeAllDevs(java.util.List, StringBuilder)
     ...
     *
     * @param program program to free up the backend resources for
     */
     ...
     * <code>lookupNet</code>.
     *
     * @param errbuf if there is an error, errbuf is filled with appropriate message
     ...
     * </p>
     *
     * @param device device to do the lookup on
     * @param netp   object which will contain the value of network address
     * @param maskp  object which will contain the value of network netmask
     * @param errbuf any error messages if return value is -1
     ...
     * @param errbuf any error messages if return value is -1
     * @return 0 on success otherwise -1 on error
     * @see #lookupNet(String, org.jnetpcap.nio.JNumber, org.jnetpcap.nio.JNumber, StringBuilder)
     ...
     * compiling BPF code.
     *
     * @param linktype pcap DLT link type integer value
     * @param snaplen  filters generated using the pcap structure will truncate captured
     ...
     * </p>
     *
     * @param device  buffer containing a C, '\0' terminated string with the the name of
     *                the device
     * @param snaplen amount of data to capture per packet; (see special note in doc
     *                comments about when this argument is ignored even when non-zero)
     * @param promisc 1 means open in promiscious mode, a 0 means non-propmiscous
     * @param timeout timeout in ms
     * @param errbuf  a buffer that will contain any error messages if the call to open
     *                failed
     ...
     * NULL.
     *
     * @param fname  filename of the pcap file
     * @param errbuf any error messages in UTC8 encoding
     ...
     * check.
     *
     * @throws PcapClosedException if pcap_t structure has been deallocated, another words if
     ...
     * @param id      numerical protocol ID found in JProtocol.ID constant and in
     *                JRegistery
     * @param handler user supplied packet handler
     * @param user    a custom opaque user object
     ...
     * @param handler user supplied packet handler
     * @param user    a custom opaque user object
     * @param scanner user supplied custom packet scanner
     * @return number of packet captured
     */
     ...
     *
     * @param cnt    number of packets to read
     * @param dumper open pcap dumper to dump all packets to
     ...
     * @param handler called when packet arrives for each packet
     * @param user    opaque user object
     * @param scanner a custom user quick-scanner for parsing headers within the packet
     * @return 0 on success, -1 on error and -2 if breakloop was used interrupt
     * the captue
     * @see ByteBufferHandler
     * @deprecated user of PcapHandler has been replaced with ByteBufferHandler
     ...
     *
     * @param fname specifies the name of the file to open; currently the libpcap
     ...
     * message.
     *
     * @param errbuf error buffer where error message will be stored on error
     ...
     *
     * @param buf contains the data of the packet to send (including the various
     *            protocol headers)
     ...
     * @param buf    contains the data of the packet to send (including the various
     *               protocol headers)
     * @param offset offset of the first index into the byte array
     * @param length amount of data to write from the offset
     * @return 0 number of bytes written otherwise -1 on failure
     */
     ...
     *
     * @param buf   buffer contains raw data to send
     ...
     *
     * @param pkt_header a packet header that will be initialized to corresponding C
     *                   structure captured values
     * @param buffer     a buffer that will be peered with returned buffer from libpcap
     ...
     * @param pkt_header a packet header that will be initialized to corresponding C
     *                   structure captured values
     * @param buffer     buffer containing packet data or null if error occured
     ...
     * </pre>
     *
     * @param packet packet that will be peered with the next libpcap returned packet
     * @return the status code
     * <ul>
     ...
     *
     * @param nonBlock a non negative value means to set in non blocking mode
     * @param errbuf   error buffer where error message will be stored on error
     * @return if there is an error, -1 is returned and errbuf is filled in with
     * an appropriate error message
     ...
     *
     * @param stats PcapStat object to fill in
